...
| `test_06_settings.py` | Trash retention (get/set/validation/persistence/actual purge via log), chunk-size setting → document count, multi-channel replication via `copyMessage`, `TELEGRAM_RATE_LIMIT` throttling (log-verified), SMTP-not-configured behaviour. |
