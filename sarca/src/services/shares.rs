...
use crate::{
    common::{access::check_access, jwt_manager::AuthUser, password_manager::PasswordManager},
...
impl<'d> SharesService<'d> {
    ...
    pub async fn create(
        ...
        check_access(&self.access, user.id, storage_id, &AccessType::W).await?;
        ...
        let link = self
            .shares
            .create(id, &token, storage_id, &path, user.id, expires_at, password_hash.as_deref())
            .await?;
    ...
    pub async fn list(
        ...
        check_access(&self.access, user.id, storage_id, &AccessType::W).await?;
        let links = self.shares.list_for_storage(storage_id).await?;
    ...
    pub async fn revoke(
        ...
        check_access(&self.access, user.id, storage_id, &AccessType::W).await?;
        self.shares.revoke(share_id, storage_id).await
    }
    ...
    async fn normalize_and_resolve_target(
        ...
        if path.ends_with('/') {
            match self.files.get_file_by_path(&path, storage_id).await {
                Ok(_) => return Ok(path),
                Err(SarcaError::DoesNotExist(_)) => {
                    let files = self.files.list_uploaded_files_under(storage_id, &path).await?;
                    if files.is_empty() {
        ...
        match self.files.get_file_by_path(&path, storage_id).await {
            ...
            Err(SarcaError::DoesNotExist(_)) => {
                ...
                match self.files.get_file_by_path(&folder, storage_id).await {
                    Ok(_) => Ok(folder),
                    Err(SarcaError::DoesNotExist(_)) => {
                        let files =
                            self.files.list_uploaded_files_under(storage_id, &folder).await?;
                        if files.is_empty() {
...
impl<'d> PublicSharesService<'d> {
    ...
    pub async fn load_available(&self, token: &str) -> SarcaResult<ShareLink> {
        let link = self.shares.get_by_token(token).await?;
    ...
    pub async fn metadata(&self, link: &ShareLink) -> SarcaResult<PublicShareMetaSchema> {
        let (name, is_file, size) = if link.is_folder() {
            ...
            let marker_ok = self.files.get_file_by_path(prefix, link.storage_id).await.is_ok();
            let files = self.files.list_uploaded_files_under(link.storage_id, prefix).await?;
            if !marker_ok && files.is_empty() {
                return Err(SarcaError::DoesNotExist("folder".to_owned()));
            }
            let size =
                self.files.sum_uploaded_size_under(link.storage_id, prefix).await.unwrap_or(0);
            (name, false, size)
        } else {
            let file = self.files.get_file_by_path(&link.path, link.storage_id).await?;
            if !file.is_uploaded {
    ...
    pub fn verify_password(link: &ShareLink, password: &str) -> SarcaResult<()> {
    ...
    /// List children under share root + relative path. Paths in response are relative to share
    /// root.
    pub async fn tree(&self, link: &ShareLink, relative: &str) -> SarcaResult<Vec<FSElement>> {
        if !link.is_folder() {
        ...
        let elements = self.files.list_dir(link.storage_id, list_prefix).await?;
        ...
        Ok(elements
            ...
            .filter_map(|el| {
                ...
                let (path, name) = if el.is_file {
                    ...
                    let p = if rel.ends_with('/') { rel } else { format!("{rel}/") };
    ...
    pub fn resolve_file_path(link: &ShareLink, relative: &str) -> SarcaResult<String> {
        if link.is_folder() {
    ...
    pub fn resolve_folder_zip_path(link: &ShareLink) -> SarcaResult<String> {
        if !link.is_folder() {
            return Err(SarcaError::InvalidPath);
        }
        Ok(link.path.clone())
    ...
    pub fn unlock_max_age_secs(link: &ShareLink) -> u64 {
        ...
        let max = link.expires_at.map_or(until_day, |exp| {
...
fn resolve_under_share(share_root: &str, relative: &str, allow_dir: bool) -> SarcaResult<String> {
...
mod tests {
    ...
    fn resolve_joins_relative() {
        ...
        assert_eq!(resolve_under_share("docs/folder/", "", true).unwrap(), "docs/folder/");
    }
}
